...
# 同时，如果使用朴素的直观想法，可以如下思考：
# 1、若s3中包含了s1和s2中没有出现的字符，则s1和s2一定无法合成s3；
# 2、若s1和s2没有重复字符，则s3是否可以合成，可以交替查询s1[i]和s2[j]是否等于s3[i+j]，并且根据s1和s2是否相等，做相应的i++或者j++即可；
# 3、若s1和s2有重复字符，上述交替查询若没有成功，应该回溯到另外一个重复字符的分支，继续查询。
